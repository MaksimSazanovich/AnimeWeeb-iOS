...
final class AuthRepository: AuthRepositoryProtocol {
    ...
    func fetchLoginConfirm(email: String, code: String) async throws -> User {
        ...
        } catch NetworkError.serverError(let statusCode, _) where statusCode == 400 {
    ...
    func fetchRegisterConfirm(email: String, code: String, nickname: String, avatar: Data) async throws -> User {
        let dto: RegisterConfirmResponse
        do {
            dto = try await networkService.request(AuthEndpoint.registerConfirm(email: email, code: code, name: nickname, avatar: avatar, deviceID: deviceID, deviceName: deviceName))
        } catch NetworkError.serverError(let statusCode, _) where statusCode == 400 {
            throw AuthError.noCode
        } catch {
            throw error
